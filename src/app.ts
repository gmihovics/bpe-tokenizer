...
const colorFor = (i: number): string => PALETTE[i % PALETTE.length] + "44";
const borderFor = (i: number): string => PALETTE[i % PALETTE.length];
...
function stepShell(num: number, title: string, desc: string, bodyHtml: string): string {
  return `
    <div class="step">
...
function renderSymbols(words: Word[]): string {
  const groups = words
    .map((w) => {
...
function render(steps: TokenizeResult): void {
  const out = el("output");
  ...
  const chunkBody = steps.chunks.length
    ? `<div class="pills">${steps.chunks
        .map(
          (c, i) =>
            `<span class="pill" style="background:${colorFor(i)};border-color:${borderFor(i)}">${esc(visible(c))}</span>`,
        )
        .join("")}</div>
  ...
  if (history.length > 0) wireScrubber(history, steps.initialWords);
}

/** Wire up the interactive merge scrubber for step 4. */
function wireScrubber(history: MergeRecord[], initialWords: Word[]): void {
  const slider = el<HTMLInputElement>("mSlider");
  const info = el("mInfo");
  const state = el("mState");
  const prev = el<HTMLButtonElement>("mPrev");
  const next = el<HTMLButtonElement>("mNext");

  // k = number of merges applied (0 .. history.length)
  function draw(k: number): void {
    if (k === 0) {
...
el("input").addEventListener("keydown", (e) => {
  ...
  if ((ke.metaKey || ke.ctrlKey) && ke.key === "Enter") run();
});
